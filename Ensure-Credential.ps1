...
$credPath = [IO.Path]::Combine($scriptDir, '.credentials.json')
...
if ($testRes.StatusCode -eq [Net.HttpStatusCode]::Unauthorized) {
  ...
  $oauthJson = [Text.Json.JsonDocument]::Parse([IO.File]::ReadAllText("$scriptDir\oauth.json"))
...
} elseif (-not $testRes.IsSuccessStatusCode) {
